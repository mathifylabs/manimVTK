...
<Tabs>
  ...
  <Tab title="Scientific Stack">
    ...
    - PyVista (advanced VTK visualization)
...
<AccordionGroup>
  ...
  <Accordion icon="triangle-exclamation" title="ModuleNotFoundError: No module named 'vtk'">
    VTK was not installed or the installation failed.
  ...
  <Accordion icon="triangle-exclamation" title="OpenGL errors on Linux">
    This occurs in headless environments (servers without display).
  ...
  <Accordion icon="triangle-exclamation" title="Permission denied errors">
    You may need to use `--user` flag or a virtual environment.
...
<Tabs>
  ...
  <Tab title="conda">
    ...
    conda create -n manimvtk python=3.11
    conda activate manimvtk
